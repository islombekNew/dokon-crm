...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const user = await prisma.user.findUnique({ where: { id: params.id }, include: { role: true, branch: true } });
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const user = await prisma.user.update({ where: { id: params.id }, data, include: { role: true, branch: true } });
    const { passwordHash, ...safe } = user;
    return NextResponse.json(successResponse(safe));
  } catch {
